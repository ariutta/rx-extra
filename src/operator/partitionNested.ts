...
export function partitionNestedInner<T>(
	...
	let [passSource, additionalStopSource] = parentPassSource.partition(passFn, thisArg);

	let stopSource: Observable<T> = Observable.merge.apply(thisArg, [parentStopSource, additionalStopSource]);
